= Docker Image for Antora (DEPRECATED)
:badges:
...
WARNING: This repository is deprecated. The functionality has been merged into https://github.com/vshn/antora[vshn/antora], which now builds directly from `node:alpine` and includes all VSHN-specific tools. Use `ghcr.io/vshn/antora` instead of `ghcr.io/vshn/docker-antora`.
